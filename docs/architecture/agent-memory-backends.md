...
On multisite, a root `convention_path` is network-shared even though section providers are loaded per request and per site. Data Machine therefore stores each site's rendered section snapshot in that site's options and deterministically merges snapshots by priority, slug, and owner blog ID. WordPress's per-blog option API can read those snapshots, but changing blog context cannot discover callbacks from plugins that were not loaded in the current request. Site-owned snapshots carry `site_id` and `site_url` provenance, and WP-CLI commands are routed with the owning URL. Recomposition replaces only the current site's snapshot, so web/cron invalidation and explicit CLI composition cannot erase another site's contribution. During migration, each site seeds its first snapshot at shutdown after its plugins have loaded; an incomplete network snapshot refuses to replace the existing root file, and the last site to seed converges the deterministic file. Single-site composition remains request-local and unchanged.
