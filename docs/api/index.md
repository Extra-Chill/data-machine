...
| Files | `/files` (multipart upload), `/files/agent`, `/files/agent/{filename}`, `/files/agent/daily`, `/files/agent/daily/{year}/{month}/{day}` | Flow-file listing/deletion moved to the `datamachine/list-flow-files` and `datamachine/delete-flow-file` abilities (#3456); upload requires a logged-in user plus `PermissionHelper::can_manage()`. Agent files allow users to access their own files; `manage_agents` can access another user's files. | `inc/Api/FlowFiles.php`, `inc/Api/AgentFiles.php` | [Files](endpoints/files.md) |
