...
`system action-scheduler-claim-index` checks for an index beginning with the exact default claim predicate/order columns: `(claim_id, status, priority, attempts, scheduled_date_gmt, action_id)`. Physical readiness always requires that exact usable index shape. Plan readiness then requires either `EXPLAIN` to use that index without filesort, allowing `LIMIT 50` to stop the ordered index walk, or an estimated candidate set of at most 50 rows. The bounded allowance covers MariaDB/MySQL choosing a cheaper alternate plan for a tiny due queue; plans above one complete claim batch must still use the ordered index without filesort. The command never builds the index during plugin bootstrap, activation, or deployment. It reports the site-scoped table, current readiness, optimizer plan, server/engine support, row and byte estimates, conservative disk requirements, and generated DDL. `--apply` uses `ALGORITHM=INPLACE, LOCK=NONE`, a bounded metadata-lock wait, an advisory migration lock, and post-DDL index and query-plan verification; unsupported runtimes fail instead of falling back to copying or blocking DDL.
