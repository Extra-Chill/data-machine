...
`system action-scheduler-claim-index` checks for an index beginning with the exact default claim predicate/order columns: `(claim_id, status, priority, attempts, scheduled_date_gmt, action_id)`. An existing index is ready only when `EXPLAIN` confirms that the default claim query uses it without filesort, allowing `LIMIT 50` to stop the ordered index walk. It never builds the index during plugin bootstrap, activation, or deployment. The command reports the site-scoped table, current readiness, optimizer plan, server/engine support, row and byte estimates, conservative disk requirements, and generated DDL. `--apply` uses `ALGORITHM=INPLACE, LOCK=NONE`, a bounded metadata-lock wait, an advisory migration lock, and post-DDL index and query-plan verification; unsupported runtimes fail instead of falling back to copying or blocking DDL.
