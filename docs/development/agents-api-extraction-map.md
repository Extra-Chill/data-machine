...
| `WP_Agent_Memory_Store` | `agents-api/inc/Core/FilesRepository/WP_Agent_Memory_Store.php` | Generic memory persistence seam consumed through the canonical Agents API memory-store resolver/filter. | Lives at `AgentsAPI\Core\FilesRepository\WP_Agent_Memory_Store`. Keep CAS/hash behavior. |
...
| `AgentMemoryStoreFactory` | `inc/Core/FilesRepository/AgentMemoryStoreFactory.php` | Data Machine-owned fallback adapter that calls `WP_Agent_Memory_Stores::get_store()` before constructing `DiskAgentMemoryStore`. | Keep host store discovery in Agents API; Data Machine owns only the disk fallback. |
