...
function datamachine_register_agents_md_sections(): void {
	...
	SectionRegistry::register(
		...
		0,
		function () use ( $wp ) {
			return '<!-- regenerated by: ' . $wp . " datamachine memory compose AGENTS.md -->\n"
	...
	if ( is_multisite() ) {
		$example_wp = preg_replace( '/--url=[^\s]+/', '--url=site.example.com', $wp );
		$example_wp = is_string( $example_wp ) ? $example_wp : $wp;
		SectionRegistry::register(
			'AGENTS.md',
			'multisite',
			40,
			function () use ( $example_wp ) {
				return "## Multisite\n\n"
...
function datamachine_agents_md_wp_cli_cmd(): string {
	...
	if ( is_multisite() ) {
		$parts[] = '--url=' . untrailingslashit( home_url( '/' ) );
	}
