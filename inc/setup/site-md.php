...
function datamachine_register_core_sections(): void {
	$snapshot_args = array(
		...
		'freshness'  => 'snapshot',
		'conditions' => 'Regenerated by composable memory invalidation or `wp datamachine memory compose`.',
	);

	// SITE.md sections — assembled into shared/SITE.md.
	SectionRegistry::register( 'SITE.md', 'header', 0, 'datamachine_site_section_header', $snapshot_args );
	SectionRegistry::register( 'SITE.md', 'identity', 10, 'datamachine_site_section_identity', $snapshot_args );
	SectionRegistry::register( 'SITE.md', 'structure', 20, 'datamachine_site_section_structure', $snapshot_args );
	SectionRegistry::register( 'SITE.md', 'post-types', 30, 'datamachine_site_section_post_types', $snapshot_args );
	SectionRegistry::register( 'SITE.md', 'taxonomies', 40, 'datamachine_site_section_taxonomies', $snapshot_args );
	SectionRegistry::register( 'SITE.md', 'user-roles', 50, 'datamachine_site_section_user_roles', $snapshot_args );
	SectionRegistry::register( 'SITE.md', 'plugins', 60, 'datamachine_site_section_plugins', $snapshot_args );
	SectionRegistry::register( 'SITE.md', 'mu-plugins', 65, 'datamachine_site_section_mu_plugins', $snapshot_args );
	SectionRegistry::register( 'SITE.md', 'dropins', 70, 'datamachine_site_section_dropins', $snapshot_args );
	SectionRegistry::register( 'SITE.md', 'rest-api', 80, 'datamachine_site_section_rest_api', $snapshot_args );

	// NETWORK.md sections — only meaningful on multisite (callbacks self-guard).
	SectionRegistry::register( 'NETWORK.md', 'header', 0, 'datamachine_network_section_header', $snapshot_args );
	SectionRegistry::register( 'NETWORK.md', 'identity', 10, 'datamachine_network_section_identity', $snapshot_args );
	SectionRegistry::register( 'NETWORK.md', 'sites', 20, 'datamachine_network_section_sites', $snapshot_args );
	SectionRegistry::register( 'NETWORK.md', 'plugins', 30, 'datamachine_network_section_plugins', $snapshot_args );
	SectionRegistry::register( 'NETWORK.md', 'shared-resources', 40, 'datamachine_network_section_shared_resources', $snapshot_args );
}
...
function datamachine_site_section_plugins(): string {
	...
	foreach ( $active_plugins as $plugin_file ) {
		...
		if ( GroupRegistrar::GROUP === strtolower( (string) $plugin_name ) || 0 === strpos( $plugin_file, GroupRegistrar::GROUP . '/' ) ) {
			continue;
		}
...
function datamachine_site_section_mu_plugins(): string {
	...
	foreach ( $mu_plugins as $mu_file => $mu_data ) {
		...
		if ( GroupRegistrar::GROUP === strtolower( $mu_name ) ) {
			continue;
		}
