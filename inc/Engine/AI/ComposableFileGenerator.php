...
class ComposableFileGenerator {
	...
	public static function regenerate( string $filename, array $context = array() ): array {
		...
		$header  = "# AI Instructions\n\n<!-- This file is auto-generated by Data Machine from registered sections. Do not edit directly. -->";
		$content = $header . "\n\n" . $content;

		// Resolve layer directory.
		...
		// Write convention copy if configured.
		$convention_written = false;
		if ( ! empty( $meta['convention_path'] ) ) {
			$convention_filepath = rtrim( ABSPATH, '/' ) . '/' . $meta['convention_path'];
			$convention_dir      = dirname( $convention_filepath );
			$convention_written  = self::write_file( $convention_filepath, $convention_dir, $content );
		}
		...
		$message = sprintf( 'Regenerated %s (%d sections).', $filename, count( SectionRegistry::get_sections( $filename ) ) );
		if ( $convention_written ) {
			$message .= sprintf( ' Convention copy written to %s.', $meta['convention_path'] );
		}
