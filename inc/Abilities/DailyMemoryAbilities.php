 ...
 * canonical Agents API memory store seam as agent-layer files under
 * `daily/YYYY/MM/DD.md`.
 *
 ...
 * selected by the canonical Agents API memory store resolver handles persistence.
 *
...
class DailyMemoryAbilities {
	 ...
	 * DailyMemory itself writes through the canonical Agents API memory store resolver; the
	...
	private static function resolveStorage( int $user_id, int $agent_id ): DailyMemoryStorage {
		 ...
		 *
		 * This filter is narrower than `wp_agent_memory_store`: it bypasses
