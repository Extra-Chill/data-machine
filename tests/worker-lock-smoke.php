...
assert_worker_lock_true( isset( $first['heartbeat_at'] ), 'acquired lock exposes an initial heartbeat' );
$before_heartbeat = (int) $first['heartbeat_at'];
assert_worker_lock_true( WorkerLock::heartbeat( (string) $first['lock_token'] ), 'owned worker heartbeat refresh succeeds' );
$heartbeat = WorkerLock::snapshot( time() + 1 );
assert_worker_lock_true( (int) $heartbeat['heartbeat_at'] >= $before_heartbeat, 'worker snapshot derives the latest lease heartbeat' );
assert_worker_lock_true( ! WorkerLock::heartbeat( 'wrong-token' ), 'wrong token cannot refresh a worker heartbeat' );
