...
$ability_provider   = datamachine_pending_actions_source( 'inc/Core/Bootstrap/AbilityServiceProvider.php' );
$host_provider      = datamachine_pending_actions_source( 'inc/Core/Bootstrap/HostIntegrationServiceProvider.php' );
...
datamachine_pending_actions_assert( str_contains( $ability_provider, 'new \\DataMachine\\Engine\\AI\\Actions\\ResolvePendingActionAbility();' ), 'existing resolve alias remains registered for compatibility', $failures, $passes );
datamachine_pending_actions_assert( str_contains( $ability_provider, 'new \\DataMachine\\Engine\\AI\\Actions\\ResolvePendingAction();' ), 'existing chat resolver tool remains registered', $failures, $passes );
...
datamachine_pending_actions_assert( str_contains( $plugin_source, 'agents_pending_action_permission' ) || str_contains( $runtime_source, 'agents_pending_action_permission' ) || str_contains( datamachine_pending_actions_source( 'inc/bootstrap.php' ), 'agents_pending_action_permission' ) || str_contains( $host_provider, 'agents_pending_action_permission' ), 'Data Machine grants canonical pending-action abilities through its chat permission policy', $failures, $passes );
datamachine_pending_actions_assert( str_contains( $ability_provider, 'PendingActionObservers::register' ) && str_contains( $ability_provider, 'WordPressActionDispatchObserver' ), 'default WordPress pending-action observer is registered during bootstrap', $failures, $passes );
...
datamachine_pending_actions_assert( str_contains( $ability_provider, 'new \\DataMachine\\Engine\\AI\\Actions\\SignPendingActionResolutionAbility();' ), 'signed pending-action resolution ability is registered', $failures, $passes );
