...
$assert( 'exact target limit one documents compound logical operations', str_contains( $ability, 'DEFAULT_APPLY_LIMIT   = 3' ) && str_contains( $cli, '--recover-pathless-children --limit=1' ) && str_contains( $cli, 'exact compound recovery may consume up to MAX_APPLY_LIMIT=100 logical touches even with --limit=1' ) );
...
$assert( 'metrics separate target attempts from logical touches and mutations', str_contains( $ability, "'target_attempts'" ) && str_contains( $ability, "'logical_touches'" ) && str_contains( $ability, "'logical_mutations'" ) );
$assert( 'limit contract distinguishes exact target and broad touch cap modes', str_contains( $ability, "'exact_target'" ) && str_contains( $ability, "'broad_touch_cap'" ) && str_contains( $ability, "'limit_unit'") );
$assert( 'exact target request preserves input and has a separate effective logical safety cap', str_contains( $ability, "'input_limit'") && str_contains( $ability, '$requested_limit = null === $job_id_scope ? $input_limit : 1' ) && str_contains( $ability, '$apply_limit     = null === $job_id_scope ? $requested_limit : self::MAX_APPLY_LIMIT' ) );
