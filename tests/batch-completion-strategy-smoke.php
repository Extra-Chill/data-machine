...
function datamachine_strategy_assert( bool $cond, string $msg ): void {
	if ( $cond ) {
		echo "  [PASS] {$msg}\n";
		return;
	}
...
function datamachine_strategy_read( string $relative_path ): string {
...
$batch_scheduler   = datamachine_strategy_read( 'inc/Core/ActionScheduler/BatchScheduler.php' );
$pipeline_batch    = datamachine_strategy_read( 'inc/Abilities/Engine/PipelineBatchScheduler.php' );
$task_scheduler    = datamachine_strategy_read( 'inc/Engine/Tasks/TaskScheduler.php' );
...
datamachine_strategy_assert( true === $metadata['batch'], 'metadata still marks the parent as a batch' );
datamachine_strategy_assert( 25 === $metadata['batch_total'], 'metadata still stores batch_total' );
datamachine_strategy_assert( 0 === $metadata['batch_scheduled'], 'metadata still initializes batch_scheduled to 0' );
datamachine_strategy_assert( 10 === $metadata['batch_chunk_size'], 'metadata still stores batch_chunk_size' );
datamachine_strategy_assert( 'pipeline' === $metadata['batch_context'], 'metadata still stores batch_context' );
datamachine_strategy_assert( $children_strategy === $metadata['batch_completion_strategy'], 'metadata stores declared completion strategy' );
...
datamachine_strategy_assert( 'task' === $metadata['batch_context'], 'task metadata still stores batch_context=task' );
datamachine_strategy_assert( $chunks_strategy === $metadata['batch_completion_strategy'], 'task metadata stores chunks_scheduled strategy' );
...
datamachine_strategy_assert( $chunks_strategy === build_task_batch_status_strategy( $metadata ), 'status helper resolves stored strategy' );
datamachine_strategy_assert( '' === build_task_batch_status_strategy( array() ), 'missing legacy strategy resolves to empty string' );
